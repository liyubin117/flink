...
public class Elasticsearch6DynamicSource implements LookupTableSource, SupportsProjectionPushDown {
	...
	public LookupRuntimeProvider getLookupRuntimeProvider(LookupContext context) {
        ...
        if (config.getUsername().isPresent()
            ...
            restClientFactory = new Elasticsearch6DynamicSink.AuthRestClientFactory(
                            config.getPathPrefix().orElse(null),
                            config.getUsername().get(),
                            config.getPassword().get());
        } else {
            restClientFactory = new Elasticsearch6DynamicSink.DefaultRestClientFactory(config.getPathPrefix().orElse(null));
        }
	...
	public DynamicTableSource copy() {
		return new Elasticsearch6DynamicSource(format, config, physicalRowDataType, lookupMaxRetryTimes, lookupCache);
	}
